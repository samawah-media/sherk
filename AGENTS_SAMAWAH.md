...
3. **لا تغيّر قلب المنتج من “المخرجات” إلى “الطلبات” بدون قرار رسمي.**
4. **لا تنفّذ جدولة السوشيال في V1.**
5. **لا تكشف تعليقات فريق سماوة الداخلية للعميل.**
6. **لا تنشئ Query أو API أو RLS Policy بدون احترام tenant/client isolation.**
7. **لا يوجد اعتماد أو رفض أو طلب تعديل بدون Audit Log.**
8. **لا يوجد SLA بدون حالة توقف عند انتظار العميل.**
9. **لا يوجد رفع ملفات بدون صلاحيات ورابط ملكية واضح.**
10. **لا تستخدم BrightBean أو Postiz أو Planka كقاعدة بناء مباشرة في V1.**
...
id
client_id / tenant_id
contract_id أو package_id اختياري
name
description
type
status
...
↓
client_approved أو client_changes_requested
```
...
1. لا يسمح بسحب كارت إلى `waiting_client_approval` إلا إذا كان `internally_approved`.
2. لا يسمح بسحب كارت إلى `delivered` إلا إذا كان `client_approved` أو لا يحتاج اعتماد عميل.
...
client_id
owner_user_id
related_deliverable_id optional
related_contract_id optional
visibility
file_type
file_size
storage_path
...
4. أي تنزيل ملف يجب أن يحترم tenant_id وpermission.
...
- `internal_comment`: فريق سماوة والإدارة فقط.
- `client_comment`: العميل + الإدارة + الفريق المصرح له.
- `system_comment`: يظهر حسب السياق.
- `approval_comment`: يظهر في سجل الاعتماد حسب نوع الاعتماد.
